...
When working with the `ViewModel` you will notice some repetetive and somewhat verbose tasks. They include calling `rebind` or configuring `rebindOnChange` to change
...
You'll notice we no longer need to pass in the `var person: Person` in the constructor. The `ItemViewModel` now has an observable property called
...
automatically rebound for you. There is also an observable `empty` boolean value you can use to check if the view model is currently holding a person.

The binding expressions need to take into account that it might not represent a specific item at the time of binding. That's why the binding
...
Most commonly you will not have both the table and the editor in the same view. We would then need to access the view model from at least two different views, one for the list and one for the form. Luckily, the ViewModel is injectable, so let's rewrite our editor example and split the two views:
...
The person list view now becomes a lot cleaner and easier to reason with. In a real application the list of persons would probably come from a controller or a remoting call though. The model is simply injected into the view, and we'll do the same for the editor:
...
## When to use ViewModel over ItemViewModel

This chapter has progressed from a verbose, non-optimal example into a pretty streamlined and maintainable piece of code, much thanks to the ItemViewModel. You might wonder if there are any use cases for inheriting from
ViewModel instead of ItemViewModel at all. The answer is that while you would typically extend ItemViewModel more than 90% of the time, there are some use cases where it doesn't make sense. Since view models can be
injected and used to keep navigational state and overall ui state, you might use it even for situations where you don't have a single domain object - you could have multiple domain objects or just a collection of loose properties.
In this use case the ItemViewModel does not make any sense, and you might as well go with the ViewModel directly. For the common case though, ItemViewModel is your best friend.
...
Here is an example that sets a new value for a fictive `updated` property of the `User` object:
...
class PersonModel(var person: Person) : ViewModel() {
    ...
    override fun onCommit() {
        person.updated = LocalDateTime.now()
    }
}
```

This is the best approach for something you want to happen on every commit. The same can be done at the call site, but would not be as DRY:

```kotlin
model.commit {
    model.person.updated = LocalDateTime.now()
}
```
