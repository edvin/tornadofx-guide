...
In some situations you might want to only receive a single event and then unsubscribe. To be able to reference the listener for later you need to save it somewhere. Here is a small application that unsubscribes after the first event is received:
...
Admittedly, this is not very convenient. Instead of the above boiler plate, you can pass the `times = n` parameter to subscribe. The event is automatically unregistered after it has been triggered `n` times:
