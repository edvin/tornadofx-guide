...
In fact, manipulating classes on several nodes at once is so common that TornadoFX provides a shortcut for it:
...
class MyApp: App(MyView::class, MyStyle::class) {
    init {
        // Only for debug - can be enabled in IDEA Plugin Run Config as well
        reloadStylesheetsOnFocus()
    }
}
```
