            ...
            right {
                form {
                    fieldset("Edit person") {
                        ...
                        button("Save") {
                            disableProperty().bind(!model.dirty)
                            action {
                                save()
                            }
                        }
...
We connect the `disabledProperty` of the button with an observable `BooleanBinding` property that is true when the model is not dirty \(notice the ! which creates a negated binding\).There is also a plain `val` called `isDirty` which returns a `Boolean` representing the dirty state for the entire model.
...
Lest's look at an example how we can retrieve only the changed objects and print them to `stdout`.
