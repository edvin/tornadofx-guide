...
For instance, define a CSS class called `critical`. Make it put a red border around any control it is applied to, and pad it by 5 pixels. 
...
Now when you apply `critical` to say, an `HBox`, all buttons inside that `HBox` will get that defined style for `button` too (Figure 6.7)
