...
>The `reloadStylesheetsOnFocus()` function call will instruct TornadoFX to reload the Stylesheets every time the `Stage` gets focus. You can also pass the `--live-stylesheets` argument to the application to accomplish the same thing. See Section X for more about startup parameter configuration.
...
But suppose when we applied `critical` to any control, such as an `HBox`, we want it to add additional stylings to buttons inside that control. Nesting another `s()` or `select()` against a `button` inside `critical` will do the trick. 
...
Now you have orange borders around the `HBox` as well as the buttons. When nesting styles, keep in mind the `s()`/`select()` will define separate styles for children, while `+s()`/`+select()` will cascade styles to children controls or classes.
