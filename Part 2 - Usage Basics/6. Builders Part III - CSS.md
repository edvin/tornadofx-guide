...
Now you have orange borders around the `HBox` as well as the buttons. When nesting styles, keep in mind the `s()`/`select()` will define separate styles for children, while `+s()`/`+select()` will cascade styles to the children.
