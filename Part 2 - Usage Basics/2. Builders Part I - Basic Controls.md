...
A `ToggleButton` is a button that expressed a true/false state depending if it is pressed or not. 
...
You can optionally pass a `ToggleGroup` to the `togglebutton()` function. This will ensure all `ToggleButton`s in that `ToggleGroup` can only have one selected at a time. 
...
A `RadioButton` is the same functionality as a `ToggleButton` but with a different visual style. 
