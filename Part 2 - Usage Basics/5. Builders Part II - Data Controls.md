...
There is an alternative syntax to creating properties that is a little nicer, but has some drawbacks. This method delegates the accessor/mutator property directly to the JavaFX Property, but it does not create a `xxxProperty()` function, which might break for code that looks for properties exposed as functions. There is nothing in TornadoFX that cares about how the properties are exposed though.
