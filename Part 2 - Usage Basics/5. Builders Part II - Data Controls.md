...
There is an alternative syntax to creating properties that is a little nicer, but has some drawbacks. This method delegates the accessor/mutator property directly to the JavaFX Property, but it does not create a `xxxProperty()` function, so reflection and some helper functions (such as the bidirectional `column("Name", Person::nameProperty)`) may not work.
