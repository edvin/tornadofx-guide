...
Creating menus can be cumbersome to build in a strictly object-oriented way. But using type-safe builders, the functional constructs make it intuitive to declare menu hierarchies. 

###MenuBar, Menu, and MenuItem

It is not uncommon to use navigable menus to keep a large number of commands organized. For instance, the `top` region of a `BorderPane` is typically where a `MenuBar` goes. There you can add menus and submenus galore (Figure 7.3)
...
You can also optionally provide key shortcuts, graphics, as well as a lambda argument for each `menuitem()` to specify the action when it is clicked (Figure 7.4).

```kotlin
menubar {
     menu("File") {
         menu("Connect") {
                           menuitem("Facebook", graphic = ImageView(Image("fb.png"))) { println("Connecting Facebook!") }
                           menuitem("Twitter", graphic = ImageView(Image("tw.png"))) { println("Connecting Twitter!") }
                       }
