...
You can also optionally provide a lambda argument to each `menuitem()` to specify the action when it is clicked. 

```kotlin
menubar {
     menu("File") {
         menu("Connect") {
             menuitem("Facebook") { println("Connecting Facebook!") }
             menuitem("Twitter") { println("Connecting Twitter!") }
         }
