...
##SingleViewApp
 ...
 If you want to quickly create an application with a single `View`, you can  choose to consolidate your `App` and `View` declarations by using a `SingleViewApp`.
...
The `SingleViewApp` is effectively an `App` and a `View` rolled into a single abstract class. You can use it to initialize the application and also treat it as the primary `View`. This can be helpful to create [SSCCE](http://sscce.org/) code snippits that can be documented and shared with others online. 
