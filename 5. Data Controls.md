...
the cell, you should consider the `ListCellFragment` instead.
...
While this might seem like slightly more work than the `cellFormat` example, this approach has a huge advantage when it comes to performance and memory usage.
You will only ever create as many fragments as you have visible listview cells on screen. The added work is definitely worth it.
