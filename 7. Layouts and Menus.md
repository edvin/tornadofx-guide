...
the tool drawers found in many popular business applications and IDEs. When an item is selected, the content for the item is displayed besides the buttons in a content area spanning the height
of the control and the preferred width of the content. When configured in `multiselect` mode it will even let you open multiple drawer items simutaneously and let them share the space available
vertical space between them. They will always open in the order of the corresponding buttons.
