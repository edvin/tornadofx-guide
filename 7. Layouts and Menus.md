...
The following code example shows how to use the `ListMenu` with the`tornadoFx` builder pattern:
```kotlin
listmenu(theme = "blue") {
...
### Item
`TornadoFx` provides the `item` builder, which allows to create `items`
for the `ListMenu` in a very convenient way. The following syntax is supported:
