...
probably does what you want by default in this regard, but you can add `fillHeight = true` to get a similar look as the Accordion.
...
The drawer is a navigation component much like a TabPane, but it organizes each drawer item in a vertically placed button bar on either the left or right side of the parent container. It resembles 
the tool drawers found in many popular business applications and IDEs. When an item is selected, the content for the item is displayed next to the buttons in a content area spanning the height
of the control and the preferred width of the content. In `multiselect` mode it will even let you open multiple drawer items simutaneously and have them share the vertical space between them. 
They will always open in the order of the corresponding buttons.

```kotlin
class DrawerView : View("Drawer Demo") {
            ...
            listview(links) {
                cellFormat { link ->
                    graphic = hyperlink(link.name) {
                        setOnAction {
                            HostServicesFactory.getInstance(FX.application).showDocument(link.uri)
                        }
                    }
                }
            }
   ...
   // Sample data variables left out
}
```

The `Workspace` features built in support for the Drawer control. The `leftDrawer` and `rightDrawer` properties of any Workspace will let you dock drawer items into them. Read more about this
