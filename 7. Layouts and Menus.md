...
TIP: The function parameter passed to the `field` builder operates on the `inputContainer`, which is a Node inside the Field where your
input elements are placed. If you were to call `hide()` on that container, only the actual inputs would be hidden. To also hide the
field label, and reclaim the space that field used to have, you must call `parent.hide()` inside the field builder.
...
In `TornadoFX`, a `ContextMenu` has functions to add `Menu` and `MenuItem` instances to it just like a `MenuBar`. It can be helpful to add a `ContextMenu` to a `TableView<Person>`, for example, and provide commands to be done on a table record (Figure 7.16). There is
